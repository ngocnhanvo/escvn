...
export const Mobile = (props: MobileProps) => {
    ...
    const page_about = props.page_about;
    ...
    return (
        /* TÍCH HỢP CLASS: block ở mobile, tự động biến mất khi màn hình >= 800px (mn-mb:hidden) */
        <div className={`absolute top-full left-0 w-full bg-white border-t border-border-subtle shadow-2xl transition-all duration-300 ease-in-out z-[60] block mn-mb:hidden ${mobileMenuOpen ? 'max-h-[90vh] opacity-100 visible' : 'max-h-0 opacity-0 invisible overflow-hidden'
            }`}>
            <nav className="flex flex-col p-4 max-h-[90vh] overflow-y-auto">
                                            ...
                                            {item.mega.map((subItem: any) => (
                                                <Link
                                                    ...
                                                    onClick={(e) => {
                                                        ...
                                                        /* FIX: Đổi từ item sang subItem để điều hướng chính xác trang con */
                                                        handlePageLink(e, subItem, `/${subItem.slug}`, navigate);
                                                    }}
                                                    to={`/${subItem.slug}`}
                                                >
                                                    {subItem.label}
                ...
                <div className="flex flex-col border-t border-border-subtle mt-2 pt-2">
                    <Link
                        ...
                        onClick={(e) => {
                            setMobileMenuOpen(false);
                            handlePageLink(e, page_about, `/${page_about.slug}`, navigate);
                        }}
                    >
                    ...
                    <Link
                        ...
                        onClick={(e) => {
                            setMobileMenuOpen(false);
                            handlePageLink(e, null, link_member, navigate);
                        }}
                    >
